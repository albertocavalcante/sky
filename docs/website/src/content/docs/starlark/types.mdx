...
<Aside type="note">
Type annotation support varies by implementation. Buck2's starlark-rust has the most advanced type system. Bazel's native starlark-java implementation has more limited support. Note: starlark-go is a separate Go implementation used by tools like Tilt and Copybara, not by Bazel.
</Aside>
...
| Feature | starlark-rust (Buck2) | starlark-java (Bazel) | starlark-go |
|---------|----------------------|----------------------|-------------|
| Basic annotations | ✅ | ✅ | ✅ |
...
<Aside type="note">
starlark-go is used by tools like Tilt, Copybara, and custom Starlark embedders. Bazel uses its own Java implementation (starlark-java).
</Aside>
