...
skylint wraps all [buildtools warnings](https://github.com/bazelbuild/buildtools/blob/master/WARNINGS.md). Use `skylint --list-rules` to see the exact categorization.

<Aside type="note">
The tables below organize rules by their logical purpose. The actual category assignment (shown in `--list-rules`) is based on rule name patterns and may differ slightly.
</Aside>
