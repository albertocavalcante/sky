...
description: Configure sky tools with sky.toml or config.sky
sidebar:
  order: 2
---
...
<Aside type="tip">
Use `sky.toml` when your configuration is straightforward. Use `config.sky` when you need conditional logic based on environment variables, OS, or architecture.
</Aside>

<Aside type="caution">
Only one config file may exist per directory. If multiple config files are found (e.g., both `config.sky` and `sky.toml`), sky will report an error.
</Aside>
...
Create a `config.sky` for dynamic configuration:
...
<TabItem label=".starlark/config.json">
```json
{
  "version": 1,
  "dialect": "bazel",
...
<Aside type="tip">
For complete dialect configuration, see the [Custom Dialects Guide](/sky/lsp/custom-dialects/). The dialect system allows you to define custom builtins for any Starlark-based tool.
</Aside>

## Related Documentation

- [skytest](/sky/tools/skytest/) - Test runner documentation
- [Configuration Schema](/sky/reference/config-schema/) - LSP dialect configuration
- [Custom Dialects](/sky/lsp/custom-dialects/) - Understanding and configuring Starlark dialects
