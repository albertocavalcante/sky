...
1. Install the [Starpls extension](https://marketplace.visualstudio.com/items?itemName=withered-magic.starpls-vscode)
2. Configure in settings:

```json
{
  "starpls.server.path": "/path/to/starpls"
}
```
...
```json
{
  "starlark.lsp.path": "buck2",
  "starlark.lsp.args": ["lsp"]
}
```

### Bazel Starlark LSP

For Bazel, use [bazel-lsp](https://github.com/ApollosProject/starlark-lsp) or configure Starpls with Bazel builtins.
...
<CardGrid>
  <Card title="Starpls" icon="seti:code">
    General Starlark support

    [Install](https://marketplace.visualstudio.com/items?itemName=withered-magic.starpls-vscode)
  </Card>
...
```bash
sky fmt path/to/file.star
sky fmt --check .  # Check without modifying
```
...
```bash
sky doc path/to/module.star -o docs/
```
...
2. **Configure VS Code**
   ...
   Install Starpls extension and configure:
   ```json
   {
     "starpls.server.path": "starpls",
     "files.associations": {
       "*.star": "starlark",
       "*.bzl": "starlark",
       "BUILD": "starlark",
