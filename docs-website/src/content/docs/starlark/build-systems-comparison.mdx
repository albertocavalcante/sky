...
| **Starlark Interpreter** | starlark-go | starlark-rust |
...
### starlark-go (Bazel) vs starlark-rust (Buck2)
...
#### starlark-go Architecture

Bazel uses [starlark-go](https://github.com/google/starlark-go), an interpreter implemented in Go:

```go
// From starlark-go documentation
// The evaluator uses a simple recursive tree walk, returning a value
// or an error for each expression.
```

**Key characteristics:**

- **Tree-walking interpreter**: Uses recursive evaluation rather than bytecode compilation
- **Go garbage collection**: Benefits from Go's GC but subject to GC pauses
- **Integer representation**: Uses `big.Int` for arbitrary precision, with small integers (less than 256) preallocated
- **Parallelism**: Independent Starlark threads can execute in parallel
- **Freezing**: All mutable values created during module initialization are frozen upon completion

<Aside type="note">
The starlark-go team experimented with JIT compilation of syntax trees to bytecode, but Go compiler limitations (no computed goto for switch statements, escape analysis issues) prevented this from outperforming the tree-walking evaluator.
</Aside>

**Performance limitations from starlark-go:**

```
// Go issue 5496: Switch statements generate binary tree of ordered
// comparisons instead of computed goto, affecting bytecode dispatch
// Go issue 20533: Escape analysis assumes make([]Value, n) always escapes
```
...
| Aspect | starlark-go | starlark-rust |
|--------|-------------|---------------|
| **Evaluation** | Tree-walking | Bytecode + optimizations |
| **Memory** | Go GC managed | Manual (no GC pauses) |
| **Integer perf** | Relatively poor (big.Int) | Optimized |
| **Parallelism** | Thread-safe with freezing | Rust Send/Sync semantics |
| **Debugging** | Limited | DAP support |

### Language Extensions Comparison

| Feature | starlark-go | starlark-rust |
|---------|-------------|---------------|
| Recursion | Flag-controlled (`-recursion`) | Supported |
| Top-level `for` | Flag-controlled | Supported |
...
| `struct` type | Via starlarkstruct package | Via record |
...
Buck2's starlark-rust implementation includes significant type system extensions not available in Bazel's starlark-go.
...
| Feature | Bazel (starlark-go) | Buck2 (starlark-rust) |
|---------|---------------------|----------------------|
...
| Projections | No | Yes (args_projections) |
...
- [facebook/starlark-rust on GitHub](https://github.com/facebook/starlark-rust)
- [bazelbuild/bazel on GitHub](https://github.com/bazelbuild/bazel)
- [google/starlark-go on GitHub](https://github.com/google/starlark-go)
- [bazelbuild/starlark on GitHub](https://github.com/bazelbuild/starlark)
